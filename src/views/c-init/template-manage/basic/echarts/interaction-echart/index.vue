...
export default {
  ...
  methods: {
    ...
    initEchart() {
        ...
        toolbox: {
          ...
          feature: {
            ...
            mySingle: {
              ...
              onclick: (e) => { this.handleEchartPointToolbox(e, 'mySingle',) }
            },
            // 多选
            myMultiple: {
              show: this.form.adjustWay === 'point',
              ...
              emphasis: { iconStyle: this.handleEchartIconStyle('emphasis'), },
              onclick: (e) => { this.handleEchartPointToolbox(e, 'myMultiple') }
            },
            // 全选
            mySelectAll: {
              show: this.form.adjustWay === 'point',
              ...
              emphasis: { iconStyle: this.handleEchartIconStyle('emphasis'), },
              onclick: (e) => { this.handleEchartPointToolbox(e, 'mySelectAll') }
            },
            // 反选
            mySelectInvert: {
              show: this.form.adjustWay === 'point',
              ...
              emphasis: { iconStyle: this.handleEchartIconStyle('emphasis'), },
              onclick: (e) => { this.handleEchartPointToolbox(e, 'mySelectInvert') }
            },
            // 单点拖拽
            mySingleDrag: {
              show: this.form.adjustWay === 'point',
              ...
              emphasis: { iconStyle: this.handleEchartIconStyle('emphasis'), },
              onclick: (e) => { this.handleEchartPointToolbox(e, 'mySingleDrag') }
            },
            // 清除
            myClear: {
              show: this.form.adjustWay === 'point',
              ...
              emphasis: { iconStyle: this.handleEchartIconStyle('emphasis'), },
              onclick: (e) => { this.handleEchartPointToolbox(e, 'myClear') }
            },
          }
        },
        tooltip: {
      ...
      console.log('查this.form.adjustWay', this.form.adjustWay)
      ...
      } else if (this.form.adjustWay === 'whole') {
        option && myChart.setOption(option)
        let selectedData = []
    ...
    handleEchartPointClick(myChart) {
      myChart.on('click', e => {
        let selectedPoints = []
        if (this.currentEchartOperate === 'mySingle') {
          selectedPoints = [{ value: e.value, series: e.seriesName }]
        } else if (this.currentEchartOperate === 'myMultiple') {
          selectedPoints = [...JSON.parse(JSON.stringify(this.selectedData)), { value: e.value, series: e.seriesName }]
        }
        this.handleSelectedData(myChart, { selectedPoints, isClick: true }, this.form.adjustWay)
      })
    },

    // echart自定义工具栏点击事件
    handleEchartPointToolbox(e, type) {
      console.log('走点击', type)
      ...
      myChart.on('brushSelected', e => {
        ...
        e.batch.forEach(batchItem => {
          batchItem.areas.forEach(areaItem => {
            ...
            //  框选顶部： areaItem.range[1][0]
            //  框选底部： areaItem.range[1][1]
            // let pixelTop = myChart.convertFromPixel({ yAxisIndex: 0 }, areaItem.range[1][0])
            // let pixelBottom = myChart.convertFromPixel({ yAxisIndex: 0 }, areaItem.range[1][1])
            // let pixelLeft = myChart.convertFromPixel({ xAxisIndex: 0 }, areaItem.range[0][0])
            // let pixelRight = myChart.convertFromPixel({ xAxisIndex: 0 }, areaItem.range[0][1])
            // console.log('查pixelTop', pixelTop)
            // console.log('查pixelBottom', pixelBottom)
            // console.log('pixelLeft', pixelLeft)
            // console.log('pixelRight', pixelRight)
            let rangeItem = {}
            // let brushedInfo = {}
    ...
    handleSelectedData(myChart, data) {
      ...
      switch (this.form.adjustWay) {
        ...
        case 'area':
          ...
          console.log('查selectedAreas1', selectedAreas)
          for (var k in xyData) {
            let filteredData = xyData[k].filter((item1, index1) => {
    ...
    // C、刷新刷选
    // handleRefreshBrushInfoList(myChart, xyData) {
    //   if (this.brushedInfoList && this.brushedInfoList[0]) {
    //     let selectedAreas = []
    //     let brushedInfoList = JSON.parse(JSON.stringify(this.brushedInfoList))
    //     let newBrushedInfoList = []
    //     brushedInfoList.forEach(item => {
    //       let rangeItem = {}
    //       let brushedInfo = {}
    ...
    //           var xMin = item.xMin
    //           var xMax = item.xMax
    //           var yMinPX = item.yMinPX
    //           var yMaxPX = item.yMaxPX
    //           var yMin = myChart.convertFromPixel({ yAxisIndex: 0 }, item.yMinPX)
    //           var yMax = myChart.convertFromPixel({ yAxisIndex: 0 }, item.yMaxPX)
    //           rangeItem = { xMin, xMax, yMin, yMax }
    //           brushedInfo = { brushType: item.brushType, xMin, xMax, yMin, yMax, yMinPX, yMaxPX }
    //           break
    //         case 'lineX':
    //           var xMin = areaItem.coordRange[0]
    //           var xMax = areaItem.coordRange[1]
    ...
    //           var yMaxPX = item.yMaxPX
    //           rangeItem = { xMin, xMax, yMin, yMax }
    //           brushedInfo = { brushType: 'lineX', xMin, xMax, yMin, yMax, yMinPX, yMaxPX }
    //           break
    //       }
    //       selectedAreas.push(rangeItem)
    //       newBrushedInfoList.push(brushedInfo)
    //     })
    //     this.$set(this, 'brushedInfoList', newBrushedInfoList)
    //     this.handleSelectedData(myChart, { selectedAreas }, 'brush',)
    //   }
    // },
    // (3) echart拖拽
